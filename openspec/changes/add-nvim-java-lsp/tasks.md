...
- [ ] 1.1 Run `brew install jdtls` and verify `command -v jdtls` resolves
  to the Homebrew-installed binary
- [ ] 1.2 Verify `java -version` (pulled in via the `openjdk` dependency)
  reports a version recent enough to run jdtls

## 2. Add the native lsp/jdtls.lua config

- [ ] 2.1 Create `nvim/.config/nvim/lsp/jdtls.lua` with `filetypes = {
...
- [ ] 2.2 Implement the per-project workspace `-data` directory: hash the
...
- [ ] 3.1 Open a `.java` file inside a project with a recognized root
  marker, run `:LspInfo`, and verify a `jdtls` client is attached with
  the expected `root_dir`
- [ ] 3.2 Verify diagnostics, completion, hover, and go-to-definition work
  in that buffer (e.g. trigger a known compile error, request hover on a
  known symbol)
- [ ] 3.3 Open a Java file from a second, unrelated project and verify
...
- [ ] 3.4 Reopen a Java file from the first project and verify it reuses
